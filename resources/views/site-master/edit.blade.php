@extends('layouts.app', ['page' => __('update site'), 'pageSlug' => 'sites'])
@section('content')
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-6"><h4 class="card-title"> Update Site</h4></div>
                </div>
            </div>
            <div class="card-body">
             <form class="form" method="post" action="{{ route('site.update',$site->id) }}">
                @csrf
                @method('put')
                <div class="card-body">
...
<div class="col-md-6 pl-md-1">
  <div class="form-group">
    <label>{{ __('Enetr and Select Client Name') }}</label>
    <input type="hidden" name="client_id" value="{{$site->client_id}}">
    <input type="text" name="client_name" class="form-control{{ $errors->has('client_name') ? ' is-invalid' : '' }}" placeholder="{{ __('Client Name') }}" value="{{$site->client_name}}" required="">
</div>
</div>
</div>
</div>
